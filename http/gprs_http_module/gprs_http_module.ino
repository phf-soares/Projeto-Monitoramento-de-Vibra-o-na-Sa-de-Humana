...
void setup() {
  ...
  init_GMS(); 
  gsm_config_gprs(); // ativar GPRS
}

void loop() {
  current_time = millis();
  if (current_time - previous_time >= AMOSTRAGEM_MS) {
    ...
    gms_http_post(url_string1, data_x);
    gms_http_post(url_string2, data_y);
    gms_http_post(url_string3, data_z);
